...
static ModuleHandleType OVR_OpenLibrary(
  ...
  ovrPlatformInitializeResult signResult = ovrPlatformInitialize_Uninitialized;
...
#define OVR_FILE_PATH_SEPARATOR "\\"
  ...
  ovrPlatformInitializeResult lastOpenResult = ovrPlatformInitialize_Uninitialized;

  {
    ...
    for (i = 0; i < sizeof(directoryArray) / sizeof(directoryArray[0]); ++i) {
      ...
      if ((printfResult >= 0) && (printfResult < (int)libraryPathCapacity)) {
          ovrPlatformInitializeResult openResult = ovrPlatformInitialize_Uninitialized;
...
OVRPL_DECLARE_IMPORT(struct ovrMessage*, ovr_PopMessage, ());
...
ovrPlatformInitializeResult loaderInitHelper(int productVersion, int majorVersion) {
  ...
  HMODULE hLibPreLoad = GetModuleHandleW(preLoadLibName);
  if (hLibPreLoad != NULL) {
  ...
  ModuleHandleType hLib = OVR_FindLibraryPath(
