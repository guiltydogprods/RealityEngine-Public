...
typedef struct ovrAvatarSpecificationRequest_ ovrAvatarSpecificationRequest;

/// The maximum joint count that can be in a skinned mesh renderer.
///
#define OVR_AVATAR_MAXIMUM_JOINT_COUNT 64

/// The maximum number of blend shapes.
///
#define OVR_AVATAR_MAXIMUM_BLEND_SHAPE_PARAM_COUNT 64
...
///
OVRN_EXPORT(ovrAvatarMessageType) ovrAvatarMessage_GetType(const ovrAvatarMessage* msg);
...
ovrAvatarMessage_GetAvatarSpecification(const ovrAvatarMessage* msg);
...
ovrAvatarMessage_GetAssetLoaded(const ovrAvatarMessage* msg);

/// Free the message when you're done processing the contents.
/// \param msg the message
///
OVRN_EXPORT(void) ovrAvatarMessage_Free(const ovrAvatarMessage* msg);
...
typedef enum ovrAvatarCapabilities_ {
  ...
  ovrAvatarCapability_BodyTilt = 1
      << 4, ///< This avatar will have a body translation with respect to the eyeOrientation
  ovrAvatarCapability_Expressive = 1 << 5, ///< This avatar will have expressive functionality
...
/// Avatar look and feel version
///
typedef enum ovrAvatarLookAndFeelVersion_ {
  ovrAvatarLookAndFeelVersion_Unknown = -1,
  ovrAvatarLookAndFeelVersion_One = 0, ///< This avatar will be the original monochromatic avatars
  ovrAvatarLookAndFeelVersion_Two = 1, ///< This avatar will be the newer skin toned avatars
...
///
OVRN_EXPORT(ovrAvatarSpecificationRequest*) ovrAvatarSpecificationRequest_Create(uint64_t userID);

/// Destroy the Specification Request
/// \param specificationRequest, the specification request to be destroyed.
///
OVRN_EXPORT(void)
ovrAvatarSpecificationRequest_Destroy(ovrAvatarSpecificationRequest* specificationRequest);
...
/// Set the Expressive Flag on the spec
/// \param request to set the field
/// \param boolean to enable expressive
///
OVRN_EXPORT(void)
...
/// Set the look and feel version on the spec
/// \param request to set the field
/// \param look and feel version
///
OVRN_EXPORT(void)
ovrAvatarSpecificationRequest_SetLookAndFeelVersion(
    ovrAvatarSpecificationRequest* request,
    const ovrAvatarLookAndFeelVersion version);

/// Update the request to include controller assets
/// \param request to set the field
/// \param boolean to enable controllers
OVRN_EXPORT(void)
ovrAvatarSpecificationRequest_SetControllersFlag(
    ovrAvatarSpecificationRequest* request,
    bool enable);
...
/// This will dispatch an async request for the avatar specification. When it's done, it will return
/// via the message system. \see ovrAvatarMessage_AvatarSpecification
///
OVRN_EXPORT(void) ovrAvatar_RequestAvatarSpecification(uint64_t userID);
...
///
OVRN_EXPORT(void)
ovrAvatar_RequestAvatarSpecificationFromSpecRequest(ovrAvatarSpecificationRequest* spec);

/// Create an avatar.
/// \param avatarSpecification the avatar specification. \see ovrAvatar_RequestAvatarSpecification
/// \param capabilities the capabilities set for this avatar. These capabilities can be
/// toggled on and off later with ovrAvatar_SetActiveCapabilities
...
typedef struct ovrAvatarHandInputState_ {
  ...
  uint32_t
      buttonMask; ///< Flags mask specifying which buttons are currently pressed \sa ovrAvatarButton
  uint32_t touchMask; ///< Flags mask specifying which portions of the controller are currently
                      ///< touched \sa ovrAvatarTouch
...
///
OVRN_EXPORT(void) ovrAvatarPose_UpdateBody(ovrAvatar* avatar, ovrAvatarTransform headPose);
...
/// Update the hand components of the avatar.
/// \param avatar an opaque pointer to the avatar state
/// \param inputStateLeft the input state of the left controller
/// \param inputStateRight the input state of the right controller
/// \param type the type of controller being updated
///
OVRN_EXPORT(void)
...
/// \param avatar an opaque pointer to the avatar state
/// \param gesture the gesture to set for the left hand. Specifying ovrAvatarHandGesture_Default
//  enables default hand animations and poses, other gestures disable them
...
/// \param avatar an opaque pointer to the avatar state
/// \param gesture the gesture to set for the right hand. Specifying ovrAvatarHandGesture_Default
/// enables default hand animations and poses, other gestures disable them
...
/// Set the left hand custom gesture. Using this disables the default hand animations. To restore
/// them, use ovrAvatar_SetLeftHandGesture with ovrAvatarHandGesture_Default.
/// \param avatar an opaque pointer to the avatar state
/// \param jointCount length of the joint array being passed in. This should be the same as the
...
ovrAvatar_SetLeftHandCustomGesture(
    ovrAvatar* avatar,
    uint32_t jointCount,
    const ovrAvatarTransform* customJointTransforms);

/// Set the right hand custom gesture. Using this disables the default hand animations. To restore
/// them, use ovrAvatar_SetRightHandGesture with ovrAvatarHandGesture_Default.
/// \param avatar an opaque pointer to the avatar state
/// \param jointCount length of the joint array being passed in. This should be the same as the
/// number of joints in the hand
/// \param customJointTransforms an array of transforms for the joints in the hand
...
/// \param avatar an opaque pointer to the avatar state
/// \param capabilities mask of currently active capabilities. Toggling a capability toggles
/// that capability being rendered for the avatar, this can be any subset of the capabilities
/// specified at creation time
/// \sa ovrAvatar_Create
///
OVRN_EXPORT(void)
ovrAvatar_SetActiveCapabilities(ovrAvatar* avatar, ovrAvatarCapabilities capabilities);
...
/// \param avatar an opaque pointer to the avatar state
/// \param index the index of the referenced asset to get. Must be between 0 and the
/// asset count - 1, inclusive
/// \return asset id
...
// The maximum number of visemes.
#define OVR_AVATAR_MAXIMUM_VISEME_PARAM_COUNT 32

/// Current viseme activation values from 0 to 1.
///
typedef struct ovrAvatarVisemes_ {
  uint32_t visemeParamCount; ///< Number of used viseme params, must be less than or equal
                             ///< too OVR_AVATAR_MAXIMUM_VISEME_PARAM_COUNT
  float visemeParams[OVR_AVATAR_MAXIMUM_VISEME_PARAM_COUNT]; ///< viseme params retrieved from
                                                             ///< OVRLipSync
} ovrAvatarVisemes;

/// Get the clothing alpha values for the combined body mesh.
/// \param Avatar to set visemes for
/// \param current viseme state from OvrLipSync
OVRN_EXPORT(void) ovrAvatar_SetVisemes(ovrAvatar* avatar, const ovrAvatarVisemes* state);
...
typedef enum ovrAvatarMaterialLayerBlendMode_ {
  ovrAvatarMaterialLayerBlendMode_Add, ///< The color value of this layer is added to the layers
                                       ///< before it.
  ovrAvatarMaterialLayerBlendMode_Multiply, ///< The color value of this layer is multiplied with
...
typedef enum ovrAvatarMaterialLayerSampleMode_ {
  ovrAvatarMaterialLayerSampleMode_Color, ///< This layer's color value comes from the color
                                          ///< constant.
  ovrAvatarMaterialLayerSampleMode_Texture, ///< This layer's color value comes from the layer's
                                            ///< texture sampler.
  ovrAvatarMaterialLayerSampleMode_TextureSingleChannel, ///< This layer's color value comes from a
                                                         ///< channel in the layer's texture sampler
                                                         ///< specified by the sample parameters.
  ovrAvatarMaterialLayerSampleMode_Parallax, ///< This layer's color value comes from the layer's
                                             ///< texture sampler with coordinates modified by the
                                             ///< material's parallax texture.
  ovrAvatarMaterialLayerSampleMode_RDSM, ///< This layer's color value comes from the layer's
                                         ///< texture sampler with coordinates modified by the
                                         ///< material's roughness texture.
...
typedef enum ovrAvatarMaterialMaskType_ {
  ...
  ovrAvatarMaterialMaskType_ViewReflection, ///< A mask is applied to this based on reflection of
                                            ///< view vector.
  ovrAvatarMaterialMaskType_Fresnel, ///< A mask is applied based on view direction's angle of
                                     ///< incidence.
...
typedef struct ovrAvatarMaterialState_ {
  ...
  ovrAvatarVector4f
      roughnessMapScaleOffset; ///< UV scale and offset parameters for the roughness map
  uint32_t layerCount; ///< Number of layers
  ovrAvatarMaterialLayerState
      layers[OVR_AVATAR_MAX_MATERIAL_LAYER_COUNT]; ///< State for each material layer
...
/// Avatar Expressive Params
typedef struct ovrAvatarExpressiveParameters_ {
  ovrAvatarVector4f irisColor;
  ovrAvatarVector4f scleraColor;
  ovrAvatarVector4f lashColor;
  ovrAvatarVector4f browColor;
  ovrAvatarVector4f lipColor;
  ovrAvatarVector4f teethColor;
  ovrAvatarVector4f gumColor;
...
typedef struct ovrAvatarSkinnedMeshPose_ {
  ...
  ovrAvatarTransform
      jointTransform[OVR_AVATAR_MAXIMUM_JOINT_COUNT]; ///< Array of local transform from parent
...
/// Blend shape parameters for a skinned mesh object
/// Blend values are in the range 0 to 1.
///
typedef struct ovrAvatarBlendShapeParams_ {
  uint32_t blendShapeParamCount; ///< Number of blend shape parameters
  float blendShapeParams[OVR_AVATAR_MAXIMUM_BLEND_SHAPE_PARAM_COUNT]; ///< Array of floats for blend
                                                                      ///< shape parameters
...
typedef enum ovrAvatarRenderPartType_ {
  ...
  ovrAvatarRenderPartType_SkinnedMeshRenderPBS, ///< Skinned Mesh Component with Physically Based
                                                ///< Renderer
...
/// Body part Types
///
typedef enum ovrAvatarBodyPartType_ {
  ovrAvatarBodyPartType_Body, ///< Body and Head. Required
  ovrAvatarBodyPartType_Clothing, ///< Clothing. Required
  ovrAvatarBodyPartType_Eyewear, ///< Eyewear. Optional on 1.75+ Avatars
  ovrAvatarBodyPartType_Hair, ///< Hair. Optional
  ovrAvatarBodyPartType_Beard, ///< Beard. Optional
  ovrAvatarBodyPartType_Count
} ovrAvatarBodyPartType;

/// Get the materials for the body
/// \param Avatar to get values from
OVRN_EXPORT(const ovrAvatarPBSMaterialState*)
ovrAvatar_GetBodyPBSMaterialStates(const ovrAvatarRenderPart* renderPart, uint32_t* count);
...
ovrAvatarRenderPart_GetType(const ovrAvatarRenderPart* renderPart);
...
ovrAvatarRenderPart_GetSkinnedMeshRender(const ovrAvatarRenderPart* renderPart);

/// Get the skinned mesh with physically based rendering part.
/// \param renderPart an opaque pointer to the render part
/// \return skinned mesh render PBS part
///
OVRN_EXPORT(const ovrAvatarRenderPart_SkinnedMeshRenderPBS*)
ovrAvatarRenderPart_GetSkinnedMeshRenderPBS(const ovrAvatarRenderPart* renderPart);

/// Get the skinned mesh with physically based rendering part.
/// \param renderPart an opaque pointer to the render part
/// \return skinned mesh render PBS V2 part
///
OVRN_EXPORT(const ovrAvatarRenderPart_SkinnedMeshRenderPBS_V2*)
ovrAvatarRenderPart_GetSkinnedMeshRenderPBSV2(const ovrAvatarRenderPart* renderPart);

/// Get the blend shape parameters associated with the skinned mesh render part.
/// \param renderPart an opaque pointer to the render part
/// \return blend shape parameters
///
OVRN_EXPORT(const ovrAvatarBlendShapeParams*)
ovrAvatarSkinnedMeshRender_GetBlendShapeParams(const ovrAvatarRenderPart* renderPart);
...
ovrAvatarRenderPart_GetProjectorRender(const ovrAvatarRenderPart* renderPart);
...
ovrAvatarComponent_Get(const ovrAvatar* avatar, uint32_t index);

/// Get the body part type for the given render part. Only valid for body component render parts.
/// \param avatar opaque pointer to the avatar
/// \param render part on a body component to get the type of.
/// \return avatar body part type
///
OVRN_EXPORT(ovrAvatarBodyPartType)
ovrAvatarRenderPart_GetBodyPartType(ovrAvatar* avatar, const ovrAvatarRenderPart* renderPart);
...
typedef struct ovrAvatarBodyComponent_ {
  ...
  ovrAvatarTransform
      centerEyeTransform; ///< Local transform for the center eye (average of the two eyes)
...
///
OVRN_EXPORT(const ovrAvatarBodyComponent*) ovrAvatarPose_GetBodyComponent(ovrAvatar* avatar);

/// Get the Base Component.
/// \param avatar opaque pointer to the avatar
/// \return the base component
///
OVRN_EXPORT(const ovrAvatarBaseComponent*) ovrAvatarPose_GetBaseComponent(ovrAvatar* avatar);
...
ovrAvatarPose_GetLeftControllerComponent(ovrAvatar* avatar);
...
///
OVRN_EXPORT(const ovrAvatarControllerComponent*)
ovrAvatarPose_GetRightControllerComponent(ovrAvatar* avatar);

/// Get the Left Hand Component.
/// \param avatar opaque pointer to the avatar
/// \return the left hand component
///
OVRN_EXPORT(const ovrAvatarHandComponent*) ovrAvatarPose_GetLeftHandComponent(ovrAvatar* avatar);

/// Get the Right Hand Component.
/// \param avatar opaque pointer to the avatar
/// \return the right hand component
///
OVRN_EXPORT(const ovrAvatarHandComponent*) ovrAvatarPose_GetRightHandComponent(ovrAvatar* avatar);
...
/// Mesh Vertex
///
typedef struct ovrAvatarBlendVertex_ {
  float x; ///< X component of the position
  float y; ///< Y component of the position
  float z; ///< Z component of the position
  float nx; ///< X component of the normal
...
typedef enum ovrAvatarTextureFormat_ {
  ...
  ovrAvatarTextureFormat_ASTC_RGB_6x6_DEPRECATED =
      3, ///< Deprecated: ASTC compressed texture. 16 bytes of header data at the beginning. No
         ///< mipmap data.
  ovrAvatarTextureFormat_ASTC_RGB_6x6_MIPMAPS =
      4, ///< ASTC compressed texture with mipmaps included directly afterwards. No header data
         ///< included.
...
///
OVRN_EXPORT(void) ovrAvatarAsset_BeginLoading(ovrAvatarAssetID assetID);

/// Get asset type.
/// \param asset opaque pointer to asset
/// \return the type of asset
///
OVRN_EXPORT(ovrAvatarAssetType) ovrAvatarAsset_GetType(const ovrAvatarAsset* asset);

/// Get mesh asset data.
/// \param asset opaque pointer to asset
/// \return mesh asset data
///
OVRN_EXPORT(const ovrAvatarMeshAssetData*) ovrAvatarAsset_GetMeshData(const ovrAvatarAsset* asset);

/// Get the blend shape count
/// \param asset opaque pointer to asset
/// \return blend shape count
///
OVRN_EXPORT(uint32_t) ovrAvatarAsset_GetMeshBlendShapeCount(const ovrAvatarAsset* asset);

/// Get the blend shape name
/// \param asset opaque pointer to asset
/// \param index of blend shape requested
/// \return blend shape name
///
OVRN_EXPORT(const char*)
ovrAvatarAsset_GetMeshBlendShapeName(const ovrAvatarAsset* asset, uint32_t blendIndex);

/// Get the blend shape vertex data
/// \param asset opaque pointer to asset
/// \return blend shape vertex data
///
OVRN_EXPORT(const ovrAvatarBlendVertex*)
ovrAvatarAsset_GetMeshBlendShapeVertices(const ovrAvatarAsset* asset);

/// Get the number of submeshes
/// \param asset opaque pointer to asset
/// \return submesh count
///
OVRN_EXPORT(uint32_t) ovrAvatarAsset_GetSubmeshCount(const ovrAvatarAsset* asset);

/// Get the index of the last submesh index
/// \param asset opaque pointer to asset
/// \param index of submesh you want offset
/// \return the index of the last geometry index of the submesh
///
OVRN_EXPORT(uint32_t)
ovrAvatarAsset_GetSubmeshLastIndex(const ovrAvatarAsset* asset, uint32_t index);

/// Get combined mesh asset data.
/// \param asset opaque pointer to asset
/// \return mesh asset data
///
OVRN_EXPORT(const ovrAvatarMeshAssetDataV2*)
ovrAvatarAsset_GetCombinedMeshData(const ovrAvatarAsset* asset);
...
///
OVRN_EXPORT(const ovrAvatarAssetID*)
ovrAvatarAsset_GetCombinedMeshIDs(const ovrAvatarAsset* asset, uint32_t* count);

OVRN_EXPORT(const ovrAvatar*) ovrAvatarAsset_GetAvatar(const ovrAvatarAsset* asset);
...
ovrAvatarAsset_GetTextureData(const ovrAvatarAsset* asset);
...
ovrAvatarAsset_GetMaterialData(const ovrAvatarAsset* assetHandle);
...
///
OVRN_EXPORT(const ovrAvatarPBSMaterialState*)
ovrAvatarAsset_GetPBSMaterialData(const ovrAvatarAsset* assetHandle);
...
///
OVRN_EXPORT(bool)
ovrAvatarPacket_Write(const ovrAvatarPacket* packet, uint32_t targetSize, uint8_t* targetBuffer);
...
OVRN_EXPORT(ovrAvatarPacket*) ovrAvatarPacket_Read(uint32_t bufferSize, BUFFER_);

/// Get duration of packet.
/// \param packet opaque pointer to the packet
/// \return duration in seconds
///
OVRN_EXPORT(float) ovrAvatarPacket_GetDurationSeconds(const ovrAvatarPacket* packet);
...
/// Integration section
///
/// These functions are used to allow csharp (unity) to access internal members without needing to
/// marshal
/////////////////////////////////////

OVRN_EXPORT(ovrAvatarTransform)
ovrAvatarSkinnedMeshRender_GetTransform(const ovrAvatarRenderPart* renderPart);
OVRN_EXPORT(ovrAvatarTransform)
ovrAvatarSkinnedMeshRenderPBS_GetTransform(const ovrAvatarRenderPart* renderPart);
OVRN_EXPORT(ovrAvatarTransform)
ovrAvatarSkinnedMeshRenderPBSV2_GetTransform(const ovrAvatarRenderPart* renderPart);
OVRN_EXPORT(uint32_t)
ovrAvatarSkinnedMeshRender_GetVisibilityMask(const ovrAvatarRenderPart* renderPart);
OVRN_EXPORT(uint32_t)
ovrAvatarSkinnedMeshRenderPBS_GetVisibilityMask(const ovrAvatarRenderPart* renderPart);
OVRN_EXPORT(uint32_t)
ovrAvatarSkinnedMeshRenderPBSV2_GetVisibilityMask(const ovrAvatarRenderPart* renderPart);
OVRN_EXPORT(bool)
ovrAvatarSkinnedMeshRender_MaterialStateChanged(const ovrAvatarRenderPart* renderPart);
OVRN_EXPORT(bool)
ovrAvatarSkinnedMeshRenderPBSV2_MaterialStateChanged(const ovrAvatarRenderPart* renderPart);
OVRN_EXPORT(ovrAvatarMaterialState)
ovrAvatarSkinnedMeshRender_GetMaterialState(const ovrAvatarRenderPart* renderPart);
OVRN_EXPORT(ovrAvatarPBSMaterialState)
ovrAvatarSkinnedMeshRenderPBSV2_GetPBSMaterialState(const ovrAvatarRenderPart* renderPart);
OVRN_EXPORT(uint64_t)
ovrAvatarSkinnedMeshRender_GetDirtyJoints(const ovrAvatarRenderPart* renderPart);
OVRN_EXPORT(uint64_t)
ovrAvatarSkinnedMeshRenderPBS_GetDirtyJoints(const ovrAvatarRenderPart* renderPart);
OVRN_EXPORT(uint64_t)
ovrAvatarSkinnedMeshRenderPBSV2_GetDirtyJoints(const ovrAvatarRenderPart* renderPart);
OVRN_EXPORT(ovrAvatarTransform)
...
ovrAvatarSkinnedMeshRenderPBS_GetAlbedoTextureAssetID(const ovrAvatarRenderPart* renderPart);
OVRN_EXPORT(ovrAvatarAssetID)
ovrAvatarSkinnedMeshRenderPBS_GetSurfaceTextureAssetID(const ovrAvatarRenderPart* renderPart);
OVRN_EXPORT(ovrAvatarExpressiveParameters)
ovrAvatar_GetExpressiveParameters(const ovrAvatar* avatar);
